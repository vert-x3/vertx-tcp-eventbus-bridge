...
`link:unavailable[JsonObject]` and return it as that type, when defined as `text/plain` the body is handler as
`link:unavailable[String]`, for the rest it is assumed to be `link:../../vertx-core/ruby/yardoc/Vertx/Buffer.html[Buffer]`.
